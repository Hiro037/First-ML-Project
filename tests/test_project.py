...
class TestIntegration:
    ...
    async def test_full_pipeline(self):
        ...
        btc_prices = pd.Series(np.random.normal(0, 0.001, 100).cumsum() + 50000, index=dates)
        eth_prices = pd.Series(1.5 * np.random.normal(0, 0.001, 100).cumsum() + 3000, index=dates)

        # Рассчитываем beta
        beta_value, eth_returns, btc_returns = calculate_beta(eth_prices, btc_prices)

        # Создаем монитор
        monitor = ResidualMonitor(beta=beta_value, threshold=0.01, window_minutes=60)
    ...
    async def test_crypto_monitor_initialization(self):
        ...
        with patch('src.main.Database') as mock_database_class, \
                patch('src.main.DatabaseManager') as mock_db_manager_class:
            ...
            mock_db_manager_instance.get_latest_beta = AsyncMock(return_value=1.5)
    ...
    async def test_initialize_no_beta(self):
        ...
        with patch('src.main.Database') as mock_database_class, \
            ...
            mock_db_manager_instance = MagicMock(spec=DatabaseManager)
            mock_db_manager_class.return_value = mock_db_manager_instance
            mock_db_manager_instance.get_latest_beta = AsyncMock(return_value=None)

            monitor = CryptoMonitor()
            with pytest.raises(ValueError, match="No beta coefficient found in database"):
                await monitor.initialize()
    ...
    async def test_crypto_monitor_trade_callback(self):
        ...
        monitor = CryptoMonitor()
        monitor.monitor = ResidualMonitor(beta=1.5, threshold=0.01, window_minutes=60)
        ...
        with patch.object(BinanceDataFetcher, 'parse_trade_message', return_value=mock_message):
            # Вызываем callback
    ...
    async def test_trade_callback_invalid_message(self):
        ...
        monitor = CryptoMonitor()
        monitor.monitor = ResidualMonitor(beta=1.5, threshold=0.01, window_minutes=60)
        with patch.object(BinanceDataFetcher, 'parse_trade_message', return_value=None):
    ...
    async def test_trade_callback_exception(self):
        ...
        monitor = CryptoMonitor()
        monitor.monitor = ResidualMonitor(beta=1.5, threshold=0.01, window_minutes=60)
        with patch.object(BinanceDataFetcher, 'parse_trade_message', side_effect=Exception("Test error")), \
                patch('src.main.logger') as mock_logger:
            await monitor.trade_callback({'data': 'test'})
        mock_logger.error.assert_called_with("Error in trade callback: Test error")

    @pytest.mark.asyncio
    async def test_trade_callback_with_alert(self):
        """Тест trade_callback с срабатыванием алерта."""
        monitor = CryptoMonitor()
        monitor.monitor = ResidualMonitor(beta=1.0, threshold=0.05, window_minutes=60)

        with patch.object(BinanceDataFetcher, 'parse_trade_message') as mock_parse, \
                patch('src.main.logger') as mock_logger, \
                patch('builtins.print') as mock_print:
            mock_parse.side_effect = [
                {'symbol': 'BTCUSDT', 'price': 100.0, 'timestamp': 1698765432000},
                {'symbol': 'ETHUSDT', 'price': 10.0, 'timestamp': 1698765432000},
                {'symbol': 'BTCUSDT', 'price': 101.0, 'timestamp': 1698765432000},
                {'symbol': 'ETHUSDT', 'price': 11.0, 'timestamp': 1698765432000}
            ]
    ...
    async def test_initialize_model(self):
        ...
        # Мокаем зависимости
        with patch('src.initialize.BinanceDataFetcher.fetch_historical_data',
                   AsyncMock(side_effect=[mock_btc_data, mock_eth_data])) as mock_fetch, \
                patch('src.initialize.DatabaseManager.insert_beta',
    ...
    async def test_beta_recalculation(self):
        ...
        monitor = CryptoMonitor()
        with patch.object(monitor, 'database', AsyncMock()), \
                patch.object(monitor, 'db_manager', MagicMock(spec=DatabaseManager)) as mock_db_manager:
            mock_db_manager.get_latest_beta = AsyncMock(return_value=1.6)

            # Мокаем initialize_model
            with patch('src.main.initialize_model', AsyncMock(return_value=1.7)):
                # Выполняем пересчет
    ...
    async def test_start_monitoring_error(self):
        ...
        with patch.object(monitor, 'initialize', AsyncMock(side_effect=Exception("Init error"))), \
