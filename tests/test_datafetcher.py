...
from src.data_fetcher import BinanceWebSocketClient, BinanceDataFetcher, get_historical_data
...
class TestDataFetcher:
    ...
    async def test_websocket_connection_and_listen(self):
        ...
        mock_websocket.__aiter__.return_value = [
            MagicMock(type=aiohttp.WSMsgType.TEXT, data=json.dumps({"e": "trade", "s": "BTCUSDT"})),
    ...
    async def test_fetch_klines_success(self, mock_get):
        ...
        mock_response = [
            [1644567890123, "50000.0", "51000.0", "49000.0", "50500.0", "1000.0",
             1644567950123, "50500000.0", 1000, "500.0", "25000000.0"]
        ]
        ...
        fetcher.rest_session = aiohttp.ClientSession()  # Просто создаем сессию, мок перехватит вызовы
    ...
    async def test_websocket_message_handling(self):
        ...
        text_msg = MagicMock(type=aiohttp.WSMsgType.TEXT, data=json.dumps({"e": "trade", "s": "BTCUSDT"}))
    ...
    async def test_context_manager(self):
        ...
        with patch('aiohttp.ClientSession', return_value=mock_session):
            async with BinanceDataFetcher() as fetcher:
                ...
                assert fetcher.session is None  # WebSocket session еще не создан
    ...
    async def test_fetch_klines_with_time_params(self, mock_get):
        ...
        klines = await fetcher.fetch_klines('BTCUSDT', '1m', 1000, start_time, end_time)
    ...
    async def test_fetch_klines_different_intervals(self, mock_get):
        ...
        fetcher = BinanceDataFetcher()
        fetcher.rest_session = aiohttp.ClientSession()  # Используем rest_session вместо session
