...
from .models import betas, prices
...
class DatabaseManager:
    ...
    async def insert_beta(self, beta_value: float, calculated_at) -> int:
        ...
        Returns:
            int: ID вставленной записи.
        """
        query = insert(betas).values(
            beta=beta_value, calculated_at=calculated_at
        )
        return await self.database.execute(query)
    ...
    async def get_latest_beta(self) -> float:
        ...
        query = (
            select(betas.c.beta).order_by(desc(betas.c.calculated_at)).limit(1)
        )
        result = await self.database.fetch_one(query)
        return float(result["beta"]) if result else None

    async def get_latest_beta_and_timestamp(
        self,
    ) -> Optional[Tuple[float, datetime]]:
        """
        Получает последнее рассчитанное значение beta
        и время его расчёта из БД.

        Returns:
            Кортеж (beta_value, calculated_at) или None,
            если запись отсутствует.
        """
        query = (
            select(betas.c.beta, betas.c.calculated_at)
            .order_by(desc(betas.c.calculated_at))
            .limit(1)
        )
        result = await self.database.fetch_one(query)
        return (
            (float(result["beta"]), result["calculated_at"])
            if result
            else None
        )
